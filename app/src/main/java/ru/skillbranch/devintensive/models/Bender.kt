...
class Bender(var status: Status = Status.NORMAL, var question: Question = Question.NAME) {
    ...
    fun listenAnswer(answer: String): Pair<String, Triple<Int, Int, Int>> {
        if (question.answers.contains(answer)) {
            ...
            return "Это не правильный ответ\n${status.nextStatus()}" to status.color
        }
    }
    ...
    enum class Question(val question: String, val answers: List<String>) {
        NAME("Как меня зовут?", listOf("бендер")) {
            override fun nextQuestion(): Question = PROFESSION
        },
        PROFESSION("Назови мою профессию?", listOf("сгибальщик", "бендер")) {
        ...
        IDLE("На этом всеб вопросов больше нет", listOf("бендер")) {
